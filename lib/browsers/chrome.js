...
// execute javascript for url
chrome.executeJavascriptUrl = function(tab) {
	javascript = 'window.prerenderData=document.URL';
	return new Promise((resolve, reject) => {
		tab.Runtime.evaluate({
			expression: javascript
		}).then((result) => {
			...
			setTimeout( () => {
				...
				tab.Runtime.evaluate({
					...
					try {
						tab.prerender.prerenderDataUrl = JSON.parse(result && result.result && result.result.value);
					} catch(e) {
						tab.prerender.prerenderDataUrl = result.result.value;
					}
					resolve();
				}).catch((err) => {
					util.log('unable to evaluate javascript on the page', err);
...
// execute javascript for visible text 
chrome.executeJavascriptBody = function(tab) {
	javascript = 'window.prerenderData=document.body.innerText';
	return new Promise((resolve, reject) => {
		tab.Runtime.evaluate({
			expression: javascript
		}).then((result) => {
			...
			setTimeout( () => {
				...
				tab.Runtime.evaluate({
					...
					try {
						tab.prerender.prerenderDataBody = JSON.parse(result && result.result && result.result.value);
					} catch(e) {
						tab.prerender.prerenderDataBody = result.result.value;
					}
					resolve();
				}).catch((err) => {
					util.log('unable to evaluate javascript on the page', err);
...
chrome.executeJavascriptPageSource = function(tab) {
	javascript = 'window.prerenderData=document.firstElementChild.outerHTML	';
	return new Promise((resolve, reject) => {
		tab.Runtime.evaluate({
			expression: javascript
		}).then((result) => {
			...
			setTimeout( () => {
				...
				tab.Runtime.evaluate({
					...
					try {
						tab.prerender.prerenderPageSource = JSON.parse(result && result.result && result.result.value);
					} catch(e) {
						tab.prerender.prerenderPageSource = result.result.value;
					}
					resolve();
				}).catch((err) => {
					util.log('unable to evaluate javascript on the page', err);
