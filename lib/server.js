...
server.onRequest = function(req, res) {
	...
	.then(() => {
		...
		return this.browser.executeJavascriptUrl(req.prerender.tab);
	}).then(() => {
		req.prerender.endLoadingUrl = new Date();
		return this.browser.executeJavascriptBody(req.prerender.tab);
	}).then(() => {
		req.prerender.endLoadingUrl = new Date();
		return this.browser.executeJavascriptPageSource(req.prerender.tab);
...
server._send = function(req, res) {
	...
	if (req.prerender.prerenderDataUrl) {
	...
	if (req.prerender.prerenderDataUrl) {
		util.log(req.prerender.prerenderDataUrl)
		res.json({
			prerenderDataUrl: req.prerender.prerenderDataUrl,
			prerenderDataBody: req.prerender.prerenderDataBody,
			prerenderDataPageSource: req.prerender.prerenderPageSource,
			content: req.prerender.content
		});
	}

	if (!req.prerender.prerenderDataUrl && req.prerender.content) {
